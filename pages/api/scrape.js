...
async function processArticle(article) {
  ...
  try {
    ...
    const hasRatingInStrong = checkContentElement.find('strong').filter(function () {
      return /Rating:/i.test($(this).text());
    }).length > 0;
    
    if (hasRatingInStrong) {
    ...
    const imdbRatings = ratingElements.toArray().map((ratingStrong) => {
      ...
      if (imdbRatingMatch) {
        ...
        if (!isNaN(imdbRating)) { // Check if it's a valid number
          return imdbRating;
        } else {
