...
import SearchSSRData from '../components/other/SearchSSRData'

const Search_Result = ({searchContents, queryString}) => {
  const [contents, setContents] = useState(searchContents.data || []);
  const [totalResult, setTotalResult] = useState(searchContents.totalData);
  ...
  const fetchMoreData = async () => {
    try {
      ...
      const response = await fetch(`/api/search_result/?query=${queryString}&page=${nextPage}`, {
        method: 'POST',
        headers: {
      ...
      if (Array.isArray(data.data)) {
        ...
        setTotalResult(data.totalData);
        if (contents.length + limit >= data.totalData) {
          setHasMore(false);
        }
      } else {
    ...
    <>
      <div>
        ...
        <h1 className="text-base">
          <strong>Movie Results for: </strong>{queryString}
        </h1>
...
export async function getServerSideProps(context) {
  ...
  try {
    const searchContents = await SearchSSRData(queryString);
    return { props: { searchContents, queryString } }; 
  } catch (error) {
    console.error("Error fetching data:", error);
